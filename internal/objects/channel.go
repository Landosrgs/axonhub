...
// RegexFilterMode defines how the regex filter rule is applied.
type RegexFilterMode string

const (
	// RegexFilterModeRegex uses a full regex pattern to match and remove content.
	// In streaming mode, this degrades to pseudo-streaming (buffers all content before filtering).
	RegexFilterModeRegex RegexFilterMode = "regex"

	// RegexFilterModeAnchor uses a prefix anchor string.
	// Content from the anchor onward is truncated.
	// Works with true streaming (truncates immediately on match).
	RegexFilterModeAnchor RegexFilterMode = "anchor"
)

// RegexFilterRule defines a single regex filter rule for channel response filtering.
type RegexFilterRule struct {
	// Pattern is the regex pattern or anchor string.
	Pattern string `json:"pattern"`

	// Mode is the filter mode: "regex" for full regex (pseudo-stream), "anchor" for prefix truncation (true stream).
	Mode RegexFilterMode `json:"mode"`
...
type ChannelSettings struct {
	...
	// RegexFilters configures response content filtering rules.
	// Supports two modes: "regex" (full regex, pseudo-stream) and "anchor" (prefix truncation, true stream).
	RegexFilters []RegexFilterRule `json:"regexFilters"`
}
