...
import { ChannelsRegexFilterDialog } from './channels-regex-filter-dialog';
    ...
    <>
      ...
      {currentRow && (
        <>
          ...
          <ChannelsRegexFilterDialog
            key={`channel-regex-filter-${currentRow.id}`}
            open={open === 'regexFilter'}
            onOpenChange={(isOpen) => {
              if (!isOpen) {
                setOpen(null);
                setTimeout(() => {
